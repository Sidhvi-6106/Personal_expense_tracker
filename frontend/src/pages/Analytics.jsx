...
const Analytics = () => {
  ...
  const analytics = useMemo(() => {
    ...
    const categories = expensesOnly.reduce(
      (acc, item) => {
        acc[item.category] =
          (acc[item.category] || 0) +
    ...
    const trends = expensesOnly.reduce((acc, item) => {
    ...
    const monthlyBuckets = expensesOnly.reduce(
      (acc, item) => {
    ...
    const recentMonths = Object.entries(monthlyBuckets)
    ...
    const currentMonthExpense =
      monthlyBuckets[monthKey(new Date())] || 0;
    ...
    const incomeTotal = incomeOnly.reduce(
      (sum, item) =>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500">
          See category splits, spending trends,
          AI suggestions, and next-month
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <span
            className={`text-sm font-semibold ${
              analytics.budgetUsage >= 1
                ? "text-rose-600"
                : analytics.budgetUsage >= 0.8
                ? "text-amber-600"
                : "text-emerald-600"
            }`}
          >
            {Math.round(
              analytics.budgetUsage * 100 || 0
            )}
            %
          </span>
      ...
      <div className="bg-slate-900 rounded-3xl p-6 text-white">
        ...
        <p className="text-slate-300 mt-3">
          {aiInsights?.summary ||
            "Track expenses regularly to unlock insights."}
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
          ...
          {(aiInsights?.suggestions?.length
            ...
            : [
                "Add more transactions to receive AI suggestions."
              ]
          ).map((item, index) => (

            <div
              key={index}
              className="rounded-2xl bg-white/5 border border-white/10 p-4 text-sm text-slate-200"
            >
              {item}
            </div>
          ...
          <div className="mt-6">
            ...
            <div className="space-y-2">
              ...
              {aiInsights.risks.map((risk, index) => (
                ...
                <div
                  key={index}
                  className="bg-rose-500/10 border border-rose-500/20 rounded-xl p-3 text-sm text-rose-200"
                >
                  {risk}
                </div>
